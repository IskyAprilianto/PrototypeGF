...
# Tampilkan prediksi cuaca Jakarta Barat
st.markdown("---")
st.subheader("🌤 Prediksi Cuaca Jakarta Barat Hari Ini")

cuaca = get_weather_forecast("Jakarta Barat", WEATHER_API_KEY)
if "error" in cuaca:
    ...
    st.markdown(f"""
        ...
        **Kecepatan Angin:** {cuaca['angin']} m/s  
    """)
                ...
                with st.expander("🔍 Analisis Kondisi", expanded=True):
                    ...
                    if latest["temperature"] > 30:
                        st.markdown('<div class="warning-box">'
                                    '⚠ <strong>Pemberitahuan: Tudung Akan Ditutup</strong><br>'
...
st.caption("🧐Catatan : Streamlit bisa saja tidak terhubung ke server (backend flask) dikarenakan running server pada Replit(webhosting yang free) auto sleep setiap 5 menit ketika tidak ada request dan ping ke server, untuk mentor atau reviewers tidak bisa mengakses dikarenakan server mati dapat menjalankan secara lokal atau bisa chat salah satu anggota kelompok agar segera menghidupkan kembali server pada Replit.")
